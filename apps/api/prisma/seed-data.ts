...
export type SeedEnvironment = Pick<
  NodeJS.ProcessEnv,
  | "NODE_ENV"
...
export async function seedBaseData(
  ...
  const defaultAdminEmail = env.DEFAULT_ADMIN_EMAIL || "admin@geo-workstation.local";
  const hasExplicitDefaultAdminPassword = Boolean(env.DEFAULT_ADMIN_PASSWORD);
  const defaultAdminPassword = env.DEFAULT_ADMIN_PASSWORD || "change_me_admin_password";
  assertSafeProductionPassword(defaultAdminPassword, hasExplicitDefaultAdminPassword, env);
...
export async function seedDemoData(
  prisma: PrismaClient,
  baseSeed: BaseSeedResult,
  env: SeedEnvironment = process.env
): Promise<void> {
  assertProductionSeedAllowed(env);
  assertDemoSeedAllowed(env);

  const { defaultCompany, defaultProductLine, admin } = baseSeed;

  const basePrompt = await findOrCreateGeoPrompt(prisma, {
    companyId: defaultCompany.id,
  ...
  const distilledPrompt = await findOrCreateGeoPrompt(prisma, {
    companyId: defaultCompany.id,
    productLineId: defaultProductLine.id,
  ...
  await findOrCreateGeoPrompt(prisma, {
    companyId: defaultCompany.id,
    productLineId: defaultProductLine.id,
  ...
  const selectionTemplate = await findOrCreateInstructionTemplate(prisma, {
    companyId: defaultCompany.id,
    name: "选型指南",
  ...
  await findOrCreateInstructionTemplate(prisma, {
    companyId: defaultCompany.id,
    name: "AI 问答素材",
...
function assertSafeProductionPassword(
  password: string,
...
async function findOrCreateGeoPrompt(
  prisma: PrismaClient,
  input: {
    companyId: string;
...
async function findOrCreateInstructionTemplate(
  prisma: PrismaClient,
  input: {
    companyId: string;
    name: string;
