...
- 文件存储：本地存储起步
- AI 接入：演示和离线开发默认使用 mock Provider，支持 OpenAI-compatible Provider；可通过后端私有环境变量配置 DeepSeek、硅基流动或其他兼容服务
- 包管理：pnpm workspace
...
- 角色与公司上下文说明：`docs/deployment/roles-and-company-context.md`
- 部署前 P1 修复清单：`docs/deployment/pre-deploy-p1-checklist.md`
...
- 发布稿富文本排版与复制：`docs/operation/publish-format-guide.md`
...
当前 mock 能力只用于演示、测试和无 Key 环境，不作为正式外部 AI 效果承诺：

- GEO 分析。
- AI 拓词默认使用 mock Provider，可切换 `openai_compatible`，并按用户决策场景生成“真实用户会向 AI 提问”的候选问题。
- GEO 内容生成默认使用 mock Provider，可切换 `openai_compatible`。

当前未实现能力：

- PC、移动网页和 App 端真实用户结果自动检测，以及全平台定时批量检测。
- 多模型批量外部检测和定时检测。
...
本阶段不做开放注册、忘记密码、短信/邮箱验证码、OAuth、多租户 SaaS、复杂审批流或细粒度菜单权限配置。完整说明见 `docs/deployment/roles-and-company-context.md`。

## AI Provider 配置

当前版本已加入统一 AI Provider 抽象：
...
当前版本支持单项目“项目档案 / 品牌档案”能力。系统不绑定某个固定公司、产品线或行业；它适用于企业品牌、产品、服务、课程、门店、本地生活、个人品牌或其他项目在生成式 AI 搜索/问答场景中的可见度运营。
...
当前自用真实流程面向本地或内网试用，不提交真实 Key，不做全自动外部 AI 检测。
...
6. 在 `/geo-content` 使用 `provider = openai_compatible` 生成内容，人工审核后导出 Markdown；`/content-tasks` 是历史兼容入口。
   内容生成后可在详情中执行“质量检查”和“生成发布优化版”，用于识别知识库外参数、品牌表达和 GEO 结构风险；还可以生成富文本发布稿，复制富文本 / Markdown / 纯文本或下载 HTML，优化稿和排版稿都不会自动覆盖原文。
7. 人工录入模型覆盖记录，并在 `/geo-reports` 复盘；`/reports` 是历史兼容入口。
...
当前部署准备已覆盖 `internal-mvp-v0.2` 的文档、环境变量示例、PM2 示例、Nginx 示例、Docker Compose 生产示例和部署检查脚本。当前仓库不连接真实服务器，不配置真实域名，不写真实数据库密码，也不会提交真实 AI Provider Key。
...
前端 API 地址读取 `VITE_API_BASE_URL`。开发环境未配置时使用本机 API；生产构建未配置时走同源 `/api`。局域网或生产部署不要把该变量设为 `localhost`，详见 `docs/deployment/env-guide.md`。

登录页：`http://localhost:5173/login`

联网检测 Provider 只在后端读取环境变量，前端不会接触 API Key。启用 `/model-inclusion-records` 的联网检测入口时，Kimi 方向需要在后端运行环境配置：
...
- `KIMI_TIMEOUT_MS`，建议设置为 `120000`

豆包 / 火山生态方向使用火山方舟 Web Search API，需要配置：

- `VOLCENGINE_WEB_SEARCH_API_KEY`
- `VOLCENGINE_WEB_SEARCH_BASE_URL`
...
通义 / 阿里云百炼方向使用百炼 OpenAI-compatible Chat Completions + Web Search，需要配置：

- `ALIYUN_BAILIAN_API_KEY`
- `ALIYUN_BAILIAN_BASE_URL`
- `ALIYUN_BAILIAN_MODEL`
- `ALIYUN_BAILIAN_WEB_SEARCH_ENABLED`
- `ALIYUN_BAILIAN_FORCE_SEARCH`
- `ALIYUN_BAILIAN_TIMEOUT_MS`，未配置时默认 `120000`
...
通义千问 / 阿里云百炼联网搜索检测写入 `platform = 通义千问 / 阿里云百炼`、`entryPoint = web_search_api`、`detectionMethod = web_search`。它是百炼 Web Search API 检测结果，不等同于通义千问 App 或网页端真实用户结果；当前响应可能不返回结构化 references / citations / searchResults / URL，第一版主要从回答正文判断品牌提及、品牌推荐和官网引用，不伪造引用来源。
...
- `/geo-content`：GEO 内容生成正式入口，支持创建默认 mock 或 `openai_compatible` 内容任务、查看/编辑/删除内容项、质量检查、发布优化版、富文本发布稿排版、Markdown/HTML 导出和失败重试入口。
- `/content-tasks`：GEO 内容生成历史兼容入口，指向同一页面。
- `/model-inclusion-records`：模型覆盖记录已升级为多入口 GEO 命中记录口径，支持手动录入、批量导入、Kimi Web Search API 联网检测、豆包 / 火山方舟联网搜索检测、通义千问 / 阿里云百炼联网搜索检测、summary、未覆盖提示词和 CSV 导出，并记录平台、入口、联网/登录状态、命中等级、内容资产引用、竞品出现、原始回答和引用来源。
...
生产或局域网部署后的 smoke 验收需要覆盖登录、`/api/auth/me`、Dashboard、GEO 诊断、提示词策略库、AI 拓词、知识库、指令库、`/geo-content` 与 `/content-tasks`、AI 模型覆盖记录、`/geo-reports` 与 `/reports`、用户管理、系统设置、上传小文件和导出小样本。smoke 账号通过环境变量注入，不使用真实敏感数据，不输出 token 或密码。
...
部署前关键批次建议使用中文业务语义 tag，例如：

```text
backup/UX-9C_全站轻量收口完成_20260517
backup/FIX-1_部署前P1修复完成_20260517
```

不要使用难以回溯业务含义的临时英文 tag，例如 `backup/before-auth-xxx`、`backup/before-user-xxx`。修复批次建议按风险分批提交，push 后再按需要创建备份 tag。

## 历史开发记录

以下 Phase 记录仅作为历史归档，帮助理解能力来源；当前交付入口、部署步骤和权限边界以上文正式说明为准。
...
- Phase Monitor-Web-3 接入通义千问 / 阿里云百炼联网搜索检测：通过百炼 OpenAI-compatible Chat Completions 启用 `enable_search` 与 `search_options.forced_search`，写入 `platform = 通义千问 / 阿里云百炼` 的联网 API 检测结果。百炼第一版定位为回答型联网检测 Provider，不等同于通义千问 App 或网页端真实用户结果；当前不稳定返回结构化引用来源，官网引用主要从回答正文判断。
...
- 不做 OpenAI、Perplexity 或 Tavily 接入；不做 PC / 移动端自动采集、App 自动化、定时任务或自动发布。
...
早期 Phase 3A 曾建议进入前端基础布局与后台框架；该目标已完成，当前后续工作以部署前 P1 修复、验收和运维文档为准。
