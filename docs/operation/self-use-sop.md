...
## 5. 配置项目档案并导入真实 GEO 提示词

先进入 `/settings` 配置项目档案。项目档案用于说明当前项目是谁、服务谁、应该用什么语气表达，适用于企业品牌、产品、服务、课程、门店、本地生活、个人品牌或其他项目。

然后再导入该项目的真实 GEO 提示词。下面的示例可替换成任意项目，不代表系统固定行业。

建议第一批提示词控制在 20-50 条，覆盖四类：

| 类型             | 示例                     |
| ---------------- | ------------------------ |
| 训练词 base      | 核心产品 / 服务 / 课程词 |
| 蒸馏词 distilled | 某产品或服务怎么选       |
| 品牌词 brand     | 某品牌或项目是否可信     |
| 场景词 scene     | 某场景适合什么方案       |
...
3. 设置：
   ...
   - `baseWord = 当前项目的核心产品词、服务词、课程词或门店场景`
...
- 候选词是否像真实用户会问 AI 的问题。
- 是否能引出品牌信任、供给方选择、需求决策、对比替代、场景方案或问题诊断。
- 是否避免无意义关键词堆砌。
- 是否没有自动入库，必须人工勾选保存。
...
- 产品 / 服务 / 课程 / 门店 / 项目简介。
- 可公开关键事实、规格、流程、服务边界或方法论。
- 典型应用、咨询、购买、到店、报名或合作场景。
- 需求决策注意事项。
- FAQ。
- 客户案例、资质、团队背景或可信证明。
...
- 明确目标提示词。
- 强调只使用知识库可验证事实。
- 要求输出用户问题、判断逻辑、项目 / 方案说明、注意事项和问答式总结。
- 禁止编造参数、资质、客户案例或外部事实。
...
在 AI 拓词的 constraints 中加入场景、用户身份、购买 / 报名 / 到店 / 合作意图或限制条件，例如“面向首次咨询用户，优先生成需求决策、顾虑解答和品牌信任类问题”。
