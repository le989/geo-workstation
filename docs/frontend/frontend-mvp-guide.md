...
- 模型覆盖记录：手动新增、批量导入、Kimi Web Search API 联网检测、豆包 / 火山方舟联网搜索检测、通义千问 / 阿里云百炼联网搜索检测、summary、未覆盖提示词、CSV 导出；已支持平台、入口、检测方式、设备类型、联网/登录状态、内容资产引用、竞品出现、命中等级、原始回答、引用来源、搜索结果、截图路径、失败原因分类和重试次数展示。选择火山 Provider 时，弹窗会提示“当前为火山方舟 Web Search API 检测”“不等同于豆包 App 端真实用户结果”“火山方舟 Web Search 可能耗时较长”“限制为短回答”“不适合生成长篇内容”和“可能不返回结构化引用来源”；选择百炼 Provider 时，弹窗会提示“当前为阿里云百炼 / 通义 Web Search API 检测”“不等同于通义千问 App 或网页端真实用户结果”“可能不返回结构化引用来源”和“官网引用主要从回答正文中判断”。
- GEO 报表：总览、覆盖报表、GEO 命中汇总、知识库覆盖、优化建议、CSV 导出；GEO 命中汇总默认使用最新结果口径，并提供平台对比、入口对比、提示词矩阵和待优化提示词清单。
