...
- 模型覆盖记录：手动新增、批量导入、Kimi Web Search API 联网检测、豆包 / 火山方舟联网搜索检测、summary、未覆盖提示词、CSV 导出；已支持平台、入口、检测方式、设备类型、联网/登录状态、内容资产引用、竞品出现、命中等级、原始回答、引用来源、搜索结果、截图路径、失败原因分类和重试次数展示。选择火山 Provider 时，弹窗会提示“当前为火山方舟 Web Search API 检测”“不等同于豆包 App 端真实用户结果”“火山方舟 Web Search 可能耗时较长”“限制为短回答”“不适合生成长篇内容”和“可能不返回结构化引用来源”。
