...
| `/model-inclusion-records` | 模型覆盖记录    | 手动录入或批量导入提示词在 AI 模型、多入口检测中的提及、推荐、官网/内容引用、竞品和命中等级。 |
...
11. 进入 `/model-inclusion-records`，人工录入或导入 GEO 命中记录，记录平台、入口、检测方式、设备、联网/登录状态、品牌是否被提及或推荐、是否引用官网/内容资产、是否出现竞品、命中等级、原始回答和引用来源。
...
- 模型覆盖记录：手动新增、批量导入、summary、未覆盖提示词、CSV 导出；已支持平台、入口、检测方式、设备类型、联网/登录状态、内容资产引用、竞品出现、命中等级、原始回答、引用来源、搜索结果和截图路径字段。
