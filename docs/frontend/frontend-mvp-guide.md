...
11. 进入 `/model-inclusion-records`，人工录入、导入 GEO 命中记录，或点击“联网检测”选择少量高优先级提示词做 Kimi Web Search API 检测。页面会记录平台、入口、检测方式、设备、联网/登录状态、品牌是否被提及或推荐、是否引用官网/内容资产、是否出现竞品、命中等级、原始回答和引用来源。该检测不等同于 Kimi App 端真实用户结果，每次检测会消耗后端 API 额度；如果出现 timeout、fetch failed 或连接重置，页面会展示失败原因分类、是否已重试，以及“网络或联网搜索超时，可稍后重试。”等提示。
...
- 模型覆盖记录：手动新增、批量导入、Kimi Web Search API 联网检测、summary、未覆盖提示词、CSV 导出；已支持平台、入口、检测方式、设备类型、联网/登录状态、内容资产引用、竞品出现、命中等级、原始回答、引用来源、搜索结果、截图路径、失败原因分类和重试次数展示。
