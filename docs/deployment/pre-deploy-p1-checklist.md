...
| P1-08 | seed 重跑可能覆盖默认管理员密码和默认公司信息            | 已处理 | `c643f11 chore: 完善部署迁移与环境配置`；生产 seed 需要显式确认；基础 seed 默认不加载演示数据        |
...
| P1-13 | seed 仅首次执行说明不足                                  | 已处理 | FIX-1D；部署、环境变量、备份文档均补充生产 seed 边界；演示 seed 需 `INCLUDE_DEMO_SEED=true` 显式开启 |
...
| M-06 | 首次 seed 策略已确认                    | 待部署确认 | 干净库只执行基础 seed；正式库不要执行演示 seed；已有真实数据后不要常规重跑 seed       |
