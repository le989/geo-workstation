...
第一版使用内部 MVP 所需的简单登录和基础角色信息，不做开放注册、多租户或复杂菜单级权限。
...
- 密码必须 hash 后存储。
- 登录态使用 JWT Bearer。
- `/health` 和登录接口可匿名访问，其他 `/api/*` 默认需要登录。
- 前端未登录访问业务页面时跳转 `/login`。
...
Phase 4D 暂不做团队管理页面、注册、忘记密码、短信/邮箱验证码、OAuth、操作审计和细粒度菜单权限。
