...
设计期曾建议中文第一版优先评估 **阿里云百炼 / 通义千问 Web Search**；实际实施中，Monitor-Web-1 先选择了 **Kimi Web Search**，Monitor-Web-2 再接入 **火山方舟 Web Search**，理由：

- Kimi `$web_search` tool-call loop 更容易观测 `search_result.search_id`，适合先跑通最小闭环。
- 火山方舟可作为豆包 / 火山生态方向的 API 检测入口，写入 `platform = 豆包 / 火山方舟`。
- 阿里云百炼 forced_search 已验证可用但结构化 URL/citations 不稳定，暂列后续 Provider。

实现层面已保留 `provider` 分发能力；后续若接阿里云百炼、OpenAI、Perplexity 或 Tavily，应继续复用 `web_search_api` 入口和 `model_inclusion_records` 字段，不把 API 检测结果误标为 App 端真实用户结果。
...
5. 升级记录字段支持 `entryPoint`、`provider`、`hitLevel`、`citations`、`searchResults` 和 `rawAnswer`。
6. 增加 PC / 移动端人工抽查入口，先保证证据字段和报表口径一致。
7. 最后考虑 Playwright/RPA 半自动化，且只在平台规则允许、账号安全可控时启用。
