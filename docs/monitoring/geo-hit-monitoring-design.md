...
| 阿里云百炼 / 通义千问 Web Search                    | 支持。Monitor-Web-3 已采用 OpenAI-compatible Chat Completions，并启用 `enable_search = true` 与 `search_options.forced_search = true`。当前实测可返回联网回答，但不稳定返回结构化 references/citations/searchResults/URL。                                                       | 中文和中国市场强。                                   | 已接入为通义 / 百炼生态方向 API 检测入口；第一版定位为“回答型联网检测 Provider”，不等同于通义千问 App 或网页端真实用户结果。                      | 中       | 支持模型、搜索触发、限流和搜索策略需在接入时固定；无结构化来源时只能从回答正文判断品牌提及、推荐和官网引用。             | 第三阶段推荐 |
| 火山方舟 Web Search（联网内容插件 / Responses API） | 支持。Monitor-Web-2 已采用火山方舟 Responses API + `tools: [{ type: "web_search" }]` 作为豆包 / 火山生态方向 API 检测入口。Monitor-Web-2.1 诊断确认网络和 Web Search 能触发，但需要控制输出 token 和回答长度；当前可能只返回 `web_search_call`，不稳定返回结构化 URL/citations。 | 中文强，适合豆包/字节生态。                          | 已接入为第二个国内 Provider，用于观察豆包 / 火山生态方向的联网 API 回答。该结果不等同于豆包 App 端真实用户结果。                                  | 中       | 插件开通、计费、限流、Responses API 响应结构变化、输出过长导致 `incomplete:length`，以及无结构化来源时只能保存联网证据。 | 第二阶段推荐 |
...
设计期曾建议中文第一版优先评估 **阿里云百炼 / 通义千问 Web Search**；实际实施中，Monitor-Web-1 先选择了 **Kimi Web Search**，Monitor-Web-2 再接入 **火山方舟 Web Search**，Monitor-Web-3 接入 **阿里云百炼 / 通义千问 Web Search**，理由：

- Kimi `$web_search` tool-call loop 更容易观测 `search_result.search_id`，适合先跑通最小闭环。
- 火山方舟可作为豆包 / 火山生态方向的 API 检测入口，写入 `platform = 豆包 / 火山方舟`。
- 阿里云百炼 forced_search 已验证可用，作为通义 / 百炼生态方向回答型联网检测 Provider；当前不稳定返回结构化 URL/citations，第一版主要依赖回答正文判断，不伪造 citations。

实现层面已保留 `provider` 分发能力；后续若接 OpenAI、Perplexity 或 Tavily，应继续复用 `web_search_api` 入口和 `model_inclusion_records` 字段，不把 API 检测结果误标为 App 端真实用户结果。
...
2. 选择一个联网搜索 Provider。当前已依次接入 Kimi、火山方舟和阿里云百炼；百炼第一版是回答型联网检测，不作为强引用来源 Provider。
