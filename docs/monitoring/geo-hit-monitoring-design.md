...
| 火山方舟 Web Search（联网内容插件 / Responses API） | 支持。Monitor-Web-2 已采用火山方舟 Responses API + `tools: [{ type: "web_search" }]` 作为豆包 / 火山生态方向 API 检测入口。当前可能只返回 `web_search_call`，不稳定返回结构化 URL/citations。 | 中文强，适合豆包/字节生态。                          | 已接入为第二个国内 Provider，用于观察豆包 / 火山生态方向的联网 API 回答。该结果不等同于豆包 App 端真实用户结果。                                  | 中       | 插件开通、计费、限流、Responses API 响应结构变化，以及无结构化来源时只能保存联网证据。        | 第二阶段推荐 |
...
4. 升级记录字段支持 `entryPoint`、`provider`、`hitLevel`、`citations`、`searchResults` 和 `rawAnswer`。
5. 增加 PC / 移动端人工抽查入口，先保证证据字段和报表口径一致。
6. 最后考虑 Playwright/RPA 半自动化，且只在平台规则允许、账号安全可控时启用。
