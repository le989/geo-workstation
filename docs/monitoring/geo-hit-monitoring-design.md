...
| Kimi Web Search                     | 支持。Kimi API 文档提供内置 `$web_search` / official tool 调用说明。                                                                                | 中文强，适合 Kimi 用户场景。                         | 适合后续增加 Kimi API 观察位，但不代表 Kimi 网页/App 端真实展示。                 | 中       | 官方工具调用形态、返回结构和可用模型需实测；网页/App 结果仍需另测。                           | 暂不第一版 |
...
本阶段不修改 Prisma schema。以下为后续 Phase Monitor-Web-1 或 Monitor-Data-1 可落地的数据模型建议。
...
建议将下一阶段命名为 **Phase Monitor-Web-1**，只完成联网搜索 API 检测闭环。
...
1. 完成 **Monitor-Web-1**：接入一个联网搜索 API，跑通“选择提示词 -> 调用联网搜索 -> 保存回答/来源 -> 自动判定 -> 报表展示”。
