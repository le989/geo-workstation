...
| POST   | `/api/content-items/:id/quality-check`        | 内容质量检查               | `provider`、`model`、`checkMode`                                                                                                                     | `score`、`level`、`riskItems`、`positiveItems`、`publishReadiness`               | 不修改原内容项；`provider=mock` 使用规则检查，`openai_compatible` 可做 AI 评审                           |
...
| POST   | `/api/model-inclusion-records/web-search-check`  | 联网 GEO 检测         | `geoPromptIds[]`、`provider = kimi_web_search \| volcengine_web_search`、`model?`、`brandName?`、`companyName?`、`websiteUrl?`、`entryPoint = web_search_api`、`isLoggedIn = false`、`limit <= 20`                                                                                                                                                                                                    | `successCount`、`failedCount`、`createdItems`、`failedItems`；联网检测返回项可包含 `retryCount`、`errorCategory` | Kimi 使用 `$web_search` tool-call loop；火山方舟使用 Responses API + `tools: [{ type: "web_search" }]`，默认要求 300 字以内短回答。火山结果写入 `platform = 豆包 / 火山方舟`，不等同于豆包 App 端真实用户结果，可能不返回结构化 URL/citations；网络类失败最多重试 1 次，单条失败不影响其他提示词。火山仅返回 `web_search_call` 但无最终回答时会返回 `provider_incomplete_output` 并保存搜索证据；响应无法解析时返回 `provider_response_parse_error` |
