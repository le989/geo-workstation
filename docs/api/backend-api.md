...
| POST   | `/api/content-items/:id/quality-check`        | 内容质量检查               | `provider`、`model`、`checkMode`                                                                                                                     | `score`、`level`、`riskItems`、`positiveItems`、`publishReadiness`、`publishStatus`、`qualityGateResult`、`qualityCheckedAt` | 持久化文章质量闸门结果；不修改标题正文；development / smoke 可用 `provider=mock`，production 禁用 mock |
