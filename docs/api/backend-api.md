...
| POST   | `/api/model-inclusion-records/web-search-check`  | Kimi 联网 GEO 检测    | `geoPromptIds[]`、`provider = kimi_web_search`、`model?`、`brandName?`、`companyName?`、`websiteUrl?`、`entryPoint = web_search_api`、`isLoggedIn = false`、`limit <= 20`                                                                                                                                                                                                                             | `successCount`、`failedCount`、`createdItems`、`failedItems`；联网检测返回项可包含 `retryCount`、`errorCategory` | 只接 Kimi；使用 `$web_search` tool-call loop；网络类失败最多重试 1 次，单条失败不影响其他提示词 |
